...
- **转发选项配置粒度优化**：
  - **问题**：原来的转发配置中，`remove_captions`、`hide_author`、`send_final_message` 三个参数是全局的，所有频道对共享相同设置
  - **改进**：将这三个参数移动到每个频道对配置中，用户现在可以为每个频道对单独配置转发选项
  - **灵活性提升**：不同的频道对可以有不同的转发策略，满足更复杂的转发需求

- **UI界面优化**：
  - **布局调整**：将"移除媒体说明文字"、"隐藏原作者"、"转发完成后发送最后一条消息"三个控件从"转发选项"标签卡移动到"频道配置"标签卡
  - **用户体验**：在频道配置界面中添加"转发选项"分组，便于用户配置每个频道对的特定选项
  - **编辑功能**：频道对编辑对话框现在也包含这三个转发选项，支持单独编辑每个频道对的配置
    ...
    "forward_channel_pairs": [
      {
        ...
        "remove_captions": true,        // 新增：移动到频道对中
        "hide_author": false,           // 新增：移动到频道对中  
        "send_final_message": true      // 新增：移动到频道对中
      }
    ],
    "forward_delay": 0.1,
    "tmp_path": "tmp",
...
- **向后兼容性**：
  - **配置迁移**：程序启动时会自动检测旧的配置格式，将全局参数迁移到每个频道对中
  - **无缝升级**：用户无需手动修改配置文件，升级后自动保持原有功能
  - **设置保持**：原有的转发设置在升级后会应用到所有现有频道对中

- **功能增强**：
  - **精细控制**：用户可以为重要频道对设置隐藏作者，而为其他频道对保留作者信息
  - **灵活策略**：不同频道对可以有不同的说明文字处理策略
  - **定制转发**：支持为特定频道对单独设置转发完成消息

### 🔧 技术实现细节

- **配置模型调整**：
  - 更新 `UIChannelPair` 类，添加三个新的布尔型字段
  - 更新 `UIForwardConfig` 类，移除三个全局参数
  - 保持默认配置的一致性和向后兼容

- **UI组件重组**：
  - 在频道配置标签卡中添加"转发选项"分组框
  - 更新添加频道对和编辑频道对的逻辑
  - 优化显示文本，包含转发选项的状态信息
