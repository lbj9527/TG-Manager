...
### ✨ v2.2.36 - 日志查看器严格当日模式 (最新版本)

**严格当日限制**：
- **📅 只显示当日日志**：完全移除历史日志文件的加载逻辑，严格只加载当日的日志文件
- **🎯 精确文件选择**：移除了固定名称的后备文件(`logs/tg_manager.log`)，避免可能的历史日志显示
- **🔒 当日保证**：确保用户在日志查看器中看到的只是当天的程序运行日志
...
today_log_paths = [
    ...
    # 移除: "logs/tg_manager.log"              # 不再使用固定名称后备文件
]
```

**用户体验改进**：
- **界面更新**：控制面板标题从"当前日志控制"改为"当日日志控制"
- **明确提示**：显示"严格只显示当日日志文件的最新2000行记录"
- **状态消息**：所有状态消息统一使用"当日日志"术语，确保用户理解

### ✨ v2.2.35 - 日志查看器显示修复 (最新版本)

**关键修复**：
- **🐛 修复日志显示问题**：解决了日志查看器只显示一条日志的关键问题
- **📁 优化文件选择逻辑**：修复日志文件路径获取逻辑，优先加载UI应用日志文件
- **📊 完整日志可见性**：从显示1-2条记录提升到显示数百条完整程序运行日志
...
**影响**：
- 用户现在能在日志查看器中看到完整的程序运行日志（包括UI初始化、客户端连接、模块加载等）
- 日志查看器显示的不再是空白或只有一条记录，而是包含数百条完整的运行日志
- 更好的问题诊断和调试体验

### ✨ v2.2.31 - 日志查看器性能优化

**日志查看器全面优化**：
- **🚀 性能提升**：限制最多显示当前日志文件的最新2000行记录，显著提高大日志文件的加载速度
- **📋 聚焦当前**：只显示当前日志文件，不再搜索和加载历史日志，避免界面卡顿
- **💾 内存优化**：通过行数限制和当前日志策略，大幅减少内存占用
- **🔄 智能读取**：使用文件尾部读取算法，只读取最新的2000行内容，避免加载整个文件
...
# 当前日志文件策略
current_log_paths = [
    f"logs/tg_forwarder_{today}.log",  # 当日转发器日志
    "logs/tg_manager.log",             # 固定名称日志
]
```

**用户体验提升**：
- **⚡ 快速加载**：即使有数万行日志记录，也能在秒级完成加载
- **📊 实时监控**：专注于当前日志文件的最新内容，更适合实时监控和调试
- **🛡️ 稳定性**：避免因加载大量历史日志导致的界面卡顿和内存不足问题
- **🔍 高效筛选**：在有限的2000行记录中进行筛选，响应速度更快
- **💡 智能提示**：界面清晰提示"最多显示当前日志文件的最新2000行记录"

**兼容性保证**：
- **🔧 功能完整**：保持所有现有功能（筛选、搜索、自动刷新、滚动等）
- **⚙️ 配置兼容**：不影响任何现有配置和设置
- **🔄 平滑升级**：从旧版本升级无需任何额外操作，自动应用优化

### ✨ v2.2.27 - 业务功能翻译系统集成

**多语言业务功能支持**：
- **🔗 转发模块翻译**：转发界面完整集成多语言翻译系统，所有UI组件支持中英文实时切换
- **📋 翻译内容扩展**：为转发、上传、下载、监听、任务管理等业务功能添加完整翻译键
- **🛠️ 集成模式标准**：建立了业务模块翻译集成的标准模式，便于其他模块参考实现
- **⚡ 实时更新**：语言切换后所有业务界面组件立即更新，包括按钮、标签、占位符、表格表头等

**转发功能翻译示例**：
```
源频道: → Source Channel:
目标频道: → Target Channels:  
开始转发 → Start Forwarding
添加频道对 → Add Channel Pair
```

**技术实现**：基于v2.2.26的完整翻译系统，扩展到主要业务功能模块，实现了转发功能的完整翻译集成。

### ✨ v2.2.26 - 多语言切换系统完整实现

**完整翻译系统框架**：
- **🔄 实时语言切换**：在设置界面切换语言后，所有UI组件立即更新翻译文本
- **📁 独立翻译文件**：使用JSON格式的翻译文件，便于维护和扩展
- **🛡️ 回退机制**：当某个翻译键缺失时，自动回退到中文翻译或显示键名

### ✨ v2.2.25 - 界面语言切换功能

**多语言支持框架**：
- **🌐 语言选择器**：在设置界面的"界面"选项卡中新增语言切换下拉框，支持8种国际语言
- **⚙️ 配置集成**：完整的配置保存和加载功能，语言设置与其他UI设置统一管理
- **🔄 同步机制**：设置界面与配置文件的语言配置完全同步，重启后保持设置
- **🛡️ 配置验证**：内置语言选项验证，确保配置文件中的语言设置有效，无效时自动回退

**支持的语言**：
- 🇨🇳 **中文** (默认) | 🇺🇸 **English** | 🇪🇸 **Español** | 🇫🇷 **Français**
...
**用户体验**：
- **📱 便捷设置**：用户可以在设置界面轻松切换界面语言
- **💾 持久保存**：语言选择自动保存到配置文件，重启后保持设置
- **🔔 友好提示**：提供"重启应用后生效"的清晰说明
- **🔧 自动修复**：配置验证机制确保语言设置始终有效

**后续规划**：
- 🔄 **多语言文本翻译**：后续版本将实现实际的多语言文本翻译功能
- 🌍 **国际化支持**：使用Qt国际化框架实现动态语言切换
- 📝 **语言包扩展**：支持自定义语言包和社区翻译贡献

### ✨ v2.2.19 - 链接检测实体类型处理修复 (最新版本)

**关键链接检测修复**：
- **🔧 重要修复**：修复了pyrogram MessageEntityType枚举处理错误导致TEXT_LINK检测失败的关键问题
- **🎯 根本解决**：改进实体类型处理逻辑，优先使用枚举的`name`属性，确保准确识别所有pyrogram实体类型
- **✅ 完全生效**：exclude_links配置现在对隐式超链接（如"点击此处"）完全有效，无遗漏
- **🔍 调试增强**：详细的类型转换日志，清楚显示原始类型和转换后类型的处理过程
- **⚡ 兼容保证**：支持pyrogram不同版本的实体类型表示方式，降级处理机制确保稳定性
...
if hasattr(raw_type, 'name'):
    entity_type = raw_type.name.lower()  # TEXT_LINK -> text_link
...
**问题解决验证**：
- **📊 检测准确性**：MessageEntityType.TEXT_LINK 现在被正确识别为 "text_link"
- **🚫 过滤生效**：包含"点击此处"等隐式超链接的消息被正确过滤
- **🔧 配置一致**：exclude_links=true 对所有链接类型（显式URL、隐式超链接、邮箱、电话）都有效
- **📈 日志透明**：通过调试日志可以清楚看到 "发现链接实体: text_link" 的确认信息

### ✨ v2.2.18 - 链接检测增强与调试

**链接检测全面增强**：
- **🔧 智能类型识别**：改进pyrogram实体类型处理，支持枚举类型、字符串类型等多种实体格式，兼容性更强
- **🔍 调试透明化**：添加详细的调试日志，实时显示检测到的实体类型和链接模式，便于问题排查
- **🎯 全面覆盖**：确保检测所有链接类型：`url`（显式链接）、`text_link`（隐式超链接）、`email`（邮箱）、`phone_number`（电话）
- **✅ 双重保障**：精确的实体检测 + 正则表达式备用检测，确保不遗漏任何链接形式
- **⚡ 错误容错**：健壮的实体类型获取机制，防止因pyrogram版本差异导致的检测失败
...
# 调试信息记录
_logger.debug(f"检测到消息实体类型: {entity_type} (原始: {entity.type})")
...
- **🔧 配置一致性**: exclude_links配置对所有消息类型和链接类型都有效，行为完全可预期
- **📊 调试友好**: 通过日志可以清楚看到链接检测的详细过程，便于排查问题
- **🛡️ 兼容保证**: 支持不同版本pyrogram的实体类型表示方式，避免版本升级问题

### ✨ v2.2.17 - 链接过滤功能完善

**链接过滤全面修复**：
- **🔧 重要修复**：修复了`exclude_links`配置对纯文本消息无效的关键问题，现在所有消息类型都遵循链接过滤规则
- **🧠 智能识别**：改进链接检测能力，能够识别Telegram中的隐式超链接（如"点击此处"这样的文本链接）
- **🎯 双重检测**：通过Telegram消息实体+正则表达式双重检测，确保不遗漏任何链接类型
- **⚡ 完整覆盖**：纯文本消息、媒体消息、媒体组全部支持精确的链接过滤

**技术实现**：
```python
# 改进的链接检测 - 支持隐式链接
def _contains_links(self, text: str, entities=None) -> bool:
    # 1. 优先检查Telegram消息实体（隐式链接）
    if entities:
...
if exclude_links:
    ...
    if self.message_filter._contains_links(message.text, message_entities):
        logger.info(f"纯文本消息 {message_id} 包含链接，根据exclude_links配置被过滤")
        continue
```

**用户体验提升**：
- **🛡️ 精确过滤**: 有效阻止包含推广链接、邀请链接的不需要消息被转发
- **📋 配置一致**: exclude_links配置对所有消息类型行为完全一致，无例外
- **🔍 智能检测**: 能识别显式URL、隐式超链接、邮箱、电话号码等多种链接形式
- **📝 清晰日志**: 被过滤的含链接消息有明确日志记录，便于了解过滤情况
- **⚙️ 向后兼容**: 修复不影响任何现有功能，纯增量改进
