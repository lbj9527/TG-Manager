...
### 增强
- **监听过滤日志视觉改进**：
  - 在过滤消息日志后自动添加50个星号(*)分隔符
  - 使过滤消息更加醒目，便于用户快速识别
  - 分隔符会同时显示在主消息面板和对应的频道标签页中
  - 提高了监听日志的可读性和用户体验

### 改进
- **UI优化**：过滤消息现在有明显的视觉分隔，更容易在日志中定位

## [1.9.77] - 2025-01-20

### 修复
- **监听过滤日志UI显示问题**：
  - 修复了消息过滤日志只在后台显示，UI界面不显示的问题
  - 在`EventEmitterMonitor`中添加了`message_filtered`信号的定义和处理
  - 现在所有过滤消息都会正确显示在监听日志界面中
  - 完善了过滤消息的信号传递机制，确保UI能及时接收并显示过滤信息

### 技术改进
- **信号系统完善**：为EventEmitterMonitor类添加了完整的message_filtered信号支持
- **调试优化**：增加了过滤消息信号的调试日志，便于问题排查
