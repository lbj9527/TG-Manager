...
#### 监听模块禁止转发处理优化
- **删除历史查询机制**：完全移除了 `get_chat_history` 历史查询逻辑，显著提升处理效率
- **采用转发模块同等策略**：实现了 **1次下载上传 + (N-1)次直接复制** 的高效模式
- **优化复制转发逻辑**：
  - 新增 `_copy_from_first_target_optimized()` 方法，直接使用 RestrictedForwardHandler 返回的消息ID进行复制
  - 新增 `_copy_filtered_from_first_target_optimized()` 方法，优化过滤后媒体组的复制转发
  - 修改 `_handle_restricted_media_group()` 返回值，增加发送消息列表参数

#### 性能提升效果
- **API调用减少**：每个禁止转发频道组合从 `1 + N*历史查询` 减少到 `1 + (N-1)*直接转发`
- **处理速度提升**：与转发模块达到相同的处理效率水平
- **内存占用优化**：减少了历史消息缓存和查询开销
- **网络请求优化**：显著减少了不必要的API请求

#### 向后兼容
- 保留了旧版方法（标记为已废弃），确保代码的向后兼容性
- 添加了警告日志，提示开发者使用新的优化版本

### 🛠️ 技术改进
- **消息ID直接传递**：利用 RestrictedForwardHandler 的返回值，避免重复查询
- **错误处理增强**：改进了异常情况下的错误处理和事件发射
- **日志优化**：增加了详细的性能相关日志信息
